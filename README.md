...
Most “systems” (services, batch jobs, agents, workflows, trading engines, AI pipelines) are ultimately a **series of actions**: validate → enrich → transform → call out to something → decide → persist → observe. The problem is that this logic often ends up scattered across classes, functions, and ad-hoc conventions.
...
In a world where a lot of code is generated by AI, pipelines also act as a *forcing function*: you can constrain generated code into a clean, composable pattern where each unit is an action with a clear input/output contract. You can mix:
- **Local actions** (pure transforms / domain logic)
- **Remote actions** (HTTP calls with sensible defaults and minimal repetition)
- **AI actions** (prompt-directed steps that compile into checked-in, testable code per target language)

This approach is driven by the creator’s 25+ years building complex Wall Street systems (AI and trading) and identifying pipelines as a highly effective structure for clarity, reusability, robustness, and long-term maintainability.
...
- **AI without chaos**: prompts stay as source-of-truth, while generated code stays reviewable, testable, and language-native.
